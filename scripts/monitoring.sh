#!/bin/bash
# Transcribed from user-shared VM script; compare with VM original before relying on exact copy.
# VM root crontab pipes stdout to wall.
arch=$(uname -a)
...
cpu_load=$(awk -v total="$delta_total" -v idle="$delta_idle" 'BEGIN {if(total>0) printf "%.1f",100*(total-idle)/total; else printf "0.0"}')
...
last_boot=$(uptime -s)
if lsblk -o TYPE | grep -q lvm; then lvm=yes; else lvm=no; fi
